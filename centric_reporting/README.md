...
Point of Sale reporting: session and daily reports, margin per product, menu
engineering and a scheduled summary email. Named for the company, not for one
vertical, so the same module installs everywhere.

## Menu Performance

`centric.reporting.menu.report` is a SQL view over `pos_order_line`, one row
...
from the product's standard price. Where that price is a BoM roll-up, **margin
here is recipe driven**; where the product is costed by hand, the report is
only as good as that figure. Products sold before costing was configured show a zero
...
A scheduled action, **Centric: daily trading summary email**, emails the
...
on: **Settings → Technical → Scheduled Actions → Centric: daily trading
...
`centric_reporting.summary_recipients` to a comma-separated list of
